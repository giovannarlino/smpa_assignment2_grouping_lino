...
payments %>% 
   group_by(disbursement_month, disbursement_year, disbursement_amount) %>% 
  summarise(newmaximum = max(disbursement_amount)) 
...
payments %>% 
   ...
   group_by(election_cycle, recipient_name, disbursement_amount) %>% 
  summarise(maximum=max(disbursement_amount)) %>% 
  summarise(max(maximum)) 


payments %>% 
   filter(election_cycle == 2020) %>% 
   group_by(election_cycle, recipient_name, disbursement_amount) %>% 
  summarise(recipient_name == max(disbursement_amount))
